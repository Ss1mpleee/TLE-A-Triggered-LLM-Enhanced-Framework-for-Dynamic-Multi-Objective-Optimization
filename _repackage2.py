#!/usr/bin/env python
"""Force-repackage to D:\新论文\ with fresh timestamps."""
import shutil
import zipfile
from pathlib import Path

SUB = Path(r'D:\新论文\论文\_submission')
ROOT = SUB.parent  # D:\新论文
...
# TLE_SWEVO_Overleaf.zip in submission/
...
with zipfile.ZipFile(overleaf_zip, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
    ...
    if fig_dir.is_dir():
        for f in sorted(fig_dir.iterdir()):
            ...
            files_added += 1
print(f"  Overleaf zip: {files_added} files, {overleaf_zip.stat().st_size / 1024 / 1024:.2f} MB")

# TLE_SWEVO_PDFs_only.zip in D:\新论文\
...
with zipfile.ZipFile(pdfs_zip, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
    ...
    for name in ['EM_UPLOAD_GUIDE.md', 'ai_audit.txt']:
        ...
        if src.exists():
            zf.write(src, arcname=name)
print(f"  PDFs-only zip: {pdfs_zip.stat().st_size / 1024 / 1024:.2f} MB")
print("Done.")
